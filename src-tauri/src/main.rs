...
use tcp_smart_socket::{SmartSocketClient, CustomError, ExecutionResult};
...
impl ManagedState {
    async fn turn_on(&mut self) -> Result<ExecutionResult, String> {
        if let Some(mut client) = self.client.take() {
            ...
            Ok(ExecutionResult::Error(
                CustomError::CommandExecutionFailure(
                    "Client not connected to remote socket. Connect first".into(),
                ),
            ))
        }
    }
    async fn connect_client(
        ...
        if self.client.is_some() {
            return Ok("Already connected. Disconnect first".to_string());
        }
...
async fn connect_client(
    addr: &str,
    state: State<'_, Mutex<ManagedState>>,
) -> Result<String, String> {
